...
from hessband import lscv_generic, select_kde_bandwidth
...
def test_analytic_vs_grid_close():
    ...
    h_analytic = select_kde_bandwidth(
        x, kernel="gauss", method="analytic", h_bounds=(0.01, 0.5)
    )
    h_grid = select_kde_bandwidth(
        x, kernel="gauss", method="grid", h_bounds=(0.01, 0.5), grid_size=40
    )
    # The analytic estimate should be close to the grid optimum
    assert abs(h_analytic - h_grid) < 0.10
